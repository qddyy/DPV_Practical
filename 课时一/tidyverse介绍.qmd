...
# 由于未被 tidyverse 引入, 需要单独加载 magrittr 包
library(magrittr)
```
...
us_rent_income %>% 
    pivot_wider(
        ...
        values_from = c(estimate, moe), # 哪些列存储了变量值
...
```{webr}
df <- tibble(
    subject = c("math", "english", "physics"),
    scores  = c("80", "78 82", "88 91 95")
)
df
```
...
```{webr}
df %>% separate_longer_delim(cols = scores, delim = " ")
```
...
```{webr}
df <- tibble(id = 1:3, x = c("20-170", "30-175", "40-180"))
df
```
...
```{webr}
df %>% separate_wider_delim(cols = x, delim = "-", names = c("age", "height"))
```
...
# slice_head/slice_tail 按数量或比例从 tibble 的头部/尾部切片
...
- [Data Tidying Cheatsheet](https://github.com/rstudio/cheatsheets/blob/main/tidyr.pdf)
